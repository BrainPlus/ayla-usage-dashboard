...
def test_methodology_describes_days_since_last_completed_session() -> None:
    ...
    assert "across bundle history" in description
    assert "No recent session" in description


def test_methodology_describes_bundle_progression_stalls_and_cadence() -> None:
    methodology = exporter.build_methodology_df("eu", "2026-05-01,2026-05-31")

    progression = methodology[
        methodology["Field"] == "Bundle progression"
    ].iloc[0]["Description"]
    stalled = methodology[
        methodology["Field"] == "Bundle stalled status"
    ].iloc[0]["Description"]
    cadence = methodology[
        methodology["Field"] == "Session cadence"
    ].iloc[0]["Description"]

    assert "actual configured session count" in progression
    assert "deduplicated Completed Sessions" in progression
    assert "30 days" in stalled
    assert "60+ day" in stalled
    assert "Average calendar days" in cadence
...
def test_excel_report_includes_bundle_progression_sheet() -> None:
    ...
    report = exporter.build_excel_report(
        pd.DataFrame(),
        pd.DataFrame(),
        pd.DataFrame(),
        "eu",
        "2026-05-01,2026-05-31",
        bundle_progression=progression,
    )

    workbook = pd.ExcelFile(io.BytesIO(report))
    assert "Bundle Progression" in workbook.sheet_names
    assert pd.read_excel(
        io.BytesIO(report), sheet_name="Bundle Progression"
    ).to_dict("records") == progression.to_dict("records")
